...
let kastenid = urlParam2("_kastenid");
...
let id = urlParam("_karteid");

document
  .getElementById("richtigButton")
  .addEventListener("click", antwortRichtig);
function antwortRichtig() {
  let fach = this.parentNode.id;
  if (fach < 4) {
    fach++;
  }
  let xhr = new XMLHttpRequest();
  xhr.responseType = `json`;
  let url = "http://localhost:8080/update-karte-lern";
  xhr.open("POST", url, true);
  xhr.setRequestHeader("Content-Type", "application/json");
  // Callback function
  xhr.onreadystatechange = function() {
    if (xhr.readyState == 4 && xhr.status == 200) {
      window.location.href = `http://localhost:8080/lern?_kastenid=${kastenid}`;
    }
  };
...
document
  ...
  .addEventListener("click", antwortFalsch);
